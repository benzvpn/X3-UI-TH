...
auto_reboot_menu() {
    clear
    echo -e "${green}====== ตั้งค่ารีบูตอัตโนมัติ ======${plain}"
    echo -e "1. เปิดรีบูตอัตโนมัติ (ทุกวันเวลา 03:00)"
    echo -e "2. ปิดรีบูตอัตโนมัติ"
    echo -e "3. ตรวจสอบสถานะ"
    echo -e "0. กลับเมนูหลัก"
    echo ""
    read -rp "กรุณาเลือกเมนู: " ar_choice
    ...
    case "$ar_choice" in
        ...
        3) check_auto_reboot ;;
        0) show_menu ;;
        *) echo -e "${red}เลือกเมนูไม่ถูกต้อง${plain}" ; sleep 1 ; auto_reboot_menu ;;
    esac
}
enable_auto_reboot() {
    (crontab -l 2>/dev/null | grep -v "/sbin/reboot" ; echo "1 * * * * /sbin/reboot") | crontab -
    echo -e "${green}เปิดรีบูตอัตโนมัติเรียบร้อย (ทุกวันเวลา 03:00)${plain}"
    sleep 2
    auto_reboot_menu
}
disable_auto_reboot() {
    crontab -l 2>/dev/null | grep -v "/sbin/reboot" | crontab -
    echo -e "${yellow}ปิดรีบูตอัตโนมัติเรียบร้อย${plain}"
    sleep 2
    auto_reboot_menu
}
check_auto_reboot() {
    if crontab -l 2>/dev/null | grep -q "/sbin/reboot"; then
        echo -e "${green}สถานะ: เปิดใช้งาน (รีบูตทุกวันเวลา 03:00)${plain}"
    else
        echo -e "${red}สถานะ: ยังไม่ได้เปิดใช้งาน${plain}"
    fi
    echo ""
    read -rp "กด Enter เพื่อกลับ..." _
    auto_reboot_menu
}
show_menu() {
    echo -e "
